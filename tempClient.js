...
var ourSessionPromise = {};
var ourSessionCipher = {};
...
startSession = async () => {
    ...
    ourSessionPromise = await sessionBuilder.processPreKey(recipientObj.key_bundle);
}

// Sending Messages: 
const encryptMessage = async (plaintext) => {

    // If we just Initiated a session, use the session we created 
    // load it (if empty )
    if (recipientAddress !== undefined) { // Possible Problem Here 
        ourSessionPromise = await store.loadSession();
    ...
    ourSessionPromise.then(function onsuccess() {
        // store.storeSession(sessionPromise); // store session 
        console.log("our session in the store is: ", store.loadSession());
        console.log("Time to send message");

        // if ourSessionCipher is undefined, create it 
        if (!ourSessionCipher) {
...
const decryptMessage = async (ciphertext) => {
    ...
    ourSessionPromise.then(function onsuccess() {
        // store.storeSession(sessionPromise); // store session 
        console.log("our session in the store is: ", store.loadSession());
        console.log("Time to RECEIVE and DECRYPT incoming message");

        ourSessionCipher = new SessionCipher(store, recipientAddress);
        console.log("Our Session Record is: ", ourSessionCipher.getRecord(recipientAddress));

        // Decrypt a normal message using an existing session
        ourSessionCipher.decryptWhisperMessage(ciphertext).then(function (plaintext) {
            // handle plaintext ArrayBuffer
            console.log("our decrypted plaintext is: ", plaintext); 
            return plaintext; 
        });
    });
}
...
startSession = () => {
    ...
    sessionPromise.then(function onsuccess() {
        ...
        // session record: 
                // identity key -> ours 
                // registration id -> ours 
        // store session takes 
                // identifier -> recipient address 
                // record -> our record 

        // TEST: retrieve session and console it out 
                // expected output: 
